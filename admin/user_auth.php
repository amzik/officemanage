...
<html>
	...
	<body class="layui-layout-body">
		...
		<script type="text/html" id="buttons">
			<a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="accountaddskuistu">添加学生订阅</a>
			<a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="accountaddskuitea">添加教师订阅</a>
        ...
        layui.use(['table','form','layer','element','jquery'], function(){
	          ...
	          var $ = layui.jquery;
	          var skui1 = "<?php echo $sku_id?>";
	          var skui2 = "<?php echo $sku_id2?>";
	            table.render({
	                elem: '#table',//表格id
	                url:"admin.php?a=invitation_code_list_auth",//list接口地址
	                ...
	                //width是宽度，不填则自动根据值的长度
	                  ...
	                  {field:'accountskuid', title: '用户订阅',align: 'center',templet:function(d){
	                  		if(d.accountskuid != ''){
	                  			if(d.accountskuid[1]){
	                  				var nowskuid1 = d.accountskuid[0]['skuId'];
	                  				var nowskuid2 = d.accountskuid[1]['skuId'];
	                  				if(skui1 == nowskuid1){
	                  					var myskuid1 = "学生订阅";
	                  				}else{
	                  					var myskuid1 = "教师订阅";
	                  				...
	                  				return myskuid1+'和'+myskuid2;
	                  			}else{
	                  				var nowskuid = d.accountskuid[0]['skuId'];
	                  				if(skui1 == nowskuid){
	                  					var myskuid = "学生订阅";
	                  				}
	                  				if(skui2 == nowskuid){
	                  ...
	                  {fixed:'right',title: '操作', width: 600, align:'center', toolbar: '#buttons'}
	                ]],
	          });
	           //监听
	          table.on('tool(table)', function(obj){
	                       ...
	                       if (res.code == 1) {
	                          ...
	                          layui.use('table', function(){
	                              ...
	                              table.reload('table', { //表格的id
	                                  url:"admin.php?a=invitation_code_list_auth",
	                              });
	                             })
	                        }
	                        layer.msg(res.msg);
	              ...
	              if(obj.event === 'accountaddskuistu'){
	                  layer.confirm('为该用户添加学生订阅吗?', function(index){
	                      $.post("admin.php?a=invitation_code_accountaddskuistu",{email:obj.data.email},function(res){
	                       if (res.code == 0) {
	                          layer.closeAll();
	                          layui.use('table', function(){
	                              var table = layui.table;
	              ...
	              if(obj.event === 'accountaddskuitea'){
	                  layer.confirm('为该用户添加教师订阅吗?', function(index){
	                      $.post("admin.php?a=invitation_code_accountaddskuitea",{email:obj.data.email},function(res){
	                       if (res.code == 0) {
	                          layer.closeAll();
	                          layui.use('table', function(){
	                              var table = layui.table;
